...
const Title = ({ level = 1, children, className = "" }) => {
  // Ensure level is a valid number between 1-6
  ...
  return React.createElement(tagName, { className: `${className} ${classes}` }, children);
};
...
const PrimaryButton = ({ children, className }) => {
  return (
    <button type="submit" className={`${className} text-white bg-primary font-medium rounded-full text-lg px-16 py-3 hover:bg-blue-700 transition ease-in-out`}>
      {children}
    </button>
  );
};
